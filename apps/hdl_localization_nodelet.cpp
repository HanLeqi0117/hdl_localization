...
public:
  ...
  void onInit() override {
    ...
    send_tf_transforms = private_nh.param<bool>("send_tf_transforms", true);
...
private:
  ...
  void points_callback(const sensor_msgs::PointCloud2ConstPtr& points_msg) {
    ...
    if(!pose_estimator) {
      NODELET_WARN_THROTTLE(5.0, "waiting for initial pose input!!");
      return;
    }

    if(!globalmap) {
      NODELET_WARN_THROTTLE(5.0, "globalmap has not been received!!");
      return;
    }
